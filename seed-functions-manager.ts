...
task("seed-functions-manager", "runs a function")
  ...
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const [functionsManagerOwner, user1, user2, user3] =
    ...
    const demoUsers = [functionsManagerOwner, user1, user2, user3].reduce(
      (
        acc: {
    ...
    if (!DEMO_OWNERS || DEMO_OWNERS.length < demos.length) {
      ...
      throw new Error(
        "Please specify DEMO_OWNERS as a comma separated string of addresses. " +
